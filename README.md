...
校园轻量化外卖单页前端系统
### 1.2 项目简介
本项目为数字媒体技术专业软件工程课程作业，基于纯HTML+原生ES6 JavaScript开发离线校园外卖演示系统，无后端、无第三方框架，依靠浏览器`localStorage`实现本地数据持久化，支持**点餐用户、校园骑手、商家申请者**三类角色完整业务流程，包含商品浏览、购物车、余额充值、下单支付、骑手接单配送、收益结算、商家入驻申请全部功能。
### 1.3 AI辅助说明
项目业务文案、文档类内容由豆包AI辅助生成；核心JS代码由Treai生成后人工调试修改；设计图、测试用例由AI生成基础模板，人工结合项目业务逻辑完善。

## 2. 运行环境
### 2.1 操作系统（支持全平台）
- Windows：Windows10 / Windows11
- MacOS：MacOS 11及以上
- Linux：Ubuntu、Deepin等桌面发行版
### 2.2 浏览器环境（核心依赖，无npm/第三方依赖包）
| 浏览器 | 最低支持版本 | 推荐版本 |
|--------|--------------|----------|
| Google Chrome | 90.0 | 120+ |
| Microsoft Edge | 90.0 | 120+ |
| Firefox | 88.0 | 115+ |
> 项目**零外部依赖**：无需Node.js、npm、Vue/React、MySQL、后端服务、网络接口，离线单机直接运行。
### 2.3 开发工具（仅修改代码时需要）
VS Code 1.80+（可选，用于代码查看、断点调试）

## 3. 安装与启动步骤
### 3.1 步骤1：获取项目文件
1. 解压项目压缩包，得到根目录文件夹`campus-food-delivery`；
2. 根目录核心文件：`index.html`（主页面+全部JS逻辑）、`README.md`、配套课程报告文档文件夹。

### 3.2 步骤2：启动运行（2种方式任选）
#### 方式一：直接本地打开（推荐，无需任何工具）
1. 进入项目根目录，右键`index.html`；
2. 打开方式选择Chrome/Edge/Firefox浏览器；
3. 页面自动加载，系统直接可用，全程无需联网。

#### 方式二：VS Code简易预览（修改代码时使用）
1. VS Code安装插件「Live Server」；
2. 用VS Code打开项目根目录，右键`index.html` → `Open with Live Server`；
3. 自动弹出浏览器页面，修改代码可实时刷新预览。

### 3.3 重置数据操作（清空所有本地账号/订单）
1. 浏览器页面按F12打开开发者工具；
2. 切换至「Application」面板 → Local Storage；
3. 清除全部`campus`前缀存储项，刷新页面数据重置。

## 4. 关键模块文件索引
本项目为单文件架构，**所有页面、样式、JS逻辑均集成在 index.html** 中，内部按功能分区，模块索引如下：
```
index.html
├─ 【HTML视图层】全部页面DOM结构（10大页面分区）
│  ├─ loginPage 登录页面
│  ├─ registerPage 注册页面
│  ├─ rolePage 角色选择页
│  ├─ orderPage 点餐首页（商品、购物车）
│  ├─ cartPanel 购物车弹窗
│  ├─ riderPage 骑手配送页面
│  ├─ profilePage 个人中心页面
│  ├─ ordersPage 全部订单筛选页面
│  ├─ walletPage 钱包充值&流水页面
│  └─ merchantPage 商家入驻申请表单
├─ 【CSS样式层】全局页面样式、按钮、卡片、弹窗布局
└─ 【JavaScript业务逻辑层】（核心代码分区）
   ├─ 1. 基础模拟数据区：foodData商品、mockOrders初始订单
   ├─ 2. 全局变量区：用户、购物车、订单、骑手统计全局存储数组
   ├─ 3. 页面路由工具函数：showPage()、updateNavActive()
   ├─ 4. 本地持久化读写模块：saveXXX() / loadXXX() 全套存储函数
   ├─ 5. 购物车管理模块：addToCart()、updateQuantity()、updateCartUI()
   ├─ 6. 商品渲染&搜索模块：renderFoodList()、搜索框监听逻辑
   ├─ 7. 用户账户&充值模块：注册登录事件、confirmRecharge()、余额计算
   ├─ 8. 订单支付流转模块：payOrder()、checkout下单结算逻辑
   ├─ 9. 骑手配送模块：acceptOrder()接单、settleRiderIncome()收益结算
   ├─ 10. 订单渲染筛选模块：renderAllOrders()、renderOrderList()
   ├─ 11. 交易流水模块：addTransaction()、renderTransactions()
   ├─ 12. 公共工具函数：showToast弹窗、renderStars评分、状态转换工具
   └─ 13. 页面初始化DOM加载事件（DOMContentLoaded）
```

## 5. 代码注释规范说明
### 5.1 注释编写规则
1. **全局模拟数据块**：顶部添加块注释，说明数组存储实体含义、字段释义；
2. **核心业务函数**：每个function顶部添加单行注释，描述函数作用、入参、输出；
3. **复杂逻辑分支**（支付扣款、骑手结算、购物车数量变更）：关键判断逻辑添加行内注释；
4. **DOM事件绑定**：按钮点击监听代码添加注释，说明触发行为；
5. **本地存储操作**：save/load系列函数统一注释，标注对应localStorage存储键名。

### 5.2 关键复杂逻辑注释示例（代码内已完整实现）
1. 购物车数量增减与总价统计逻辑；
2. 订单支付余额校验、扣款、多状态订单同步逻辑；
3. 骑手接单、当日收益累加、收入结算流转逻辑；
4. 用户注册表单非空、密码一致性、账号唯一性校验；
5. 本地存储页面刷新自动加载、数据持久化逻辑；
6. 订单四状态（待付款/待接单/配送中/已完成）渲染、筛选、状态同步。

### 5.3 代码快速查阅指引
1. 查看商品/订单模拟数据：文件JS顶部 `foodData`、`mockOrders`；
2. 查看购物车核心逻辑：`addToCart()`、`updateQuantity()` 函数；
3. 查看下单支付核心逻辑：`payOrder()`、`checkoutBtn` 点击事件；
4. 查看骑手接单结算逻辑：`acceptOrder()`、`settleRiderIncome()`；
5. 查看本地存储读写：所有 `save` / `load` 前缀函数；
6. 查看页面切换路由：`showPage()` 函数。

## 6. 项目使用说明
1. 初始测试账号：手机号`13800001111`，密码`123456`；
2. 新用户可在注册页面自由创建账号，数据自动保存在浏览器本地；
3. 系统分为三类角色入口，登录后可切换使用点餐、骑手、商家入驻功能；
4. 所有资金、订单数据仅本地缓存，清除浏览器缓存后数据全部重置，无云端留存。

## 7. 配套课程文档索引
项目根目录附带软件工程全套课程报告：
1. 4.1 可行性分析报告.docx
2. 4.2 需求规格说明书(SRS).docx
3. 4.3 软件设计文档(SDD).docx
4. 4.5 软件测试报告.docx
